...
void RegisterBlock(int id, int top, int bottom, int sides1, int side2, int side3, int side4) {
    blockLibrary[id].faces[0] = sides1;  
    blockLibrary[id].faces[1] = side2; 
    blockLibrary[id].faces[2] = side3;  
    blockLibrary[id].faces[3] = side4;  
    blockLibrary[id].faces[4] = bottom; 
    blockLibrary[id].faces[5] = top;  
}
    ...
    for (int x = 0; x < chunksize; x++) {
        for (int y = 0; y < chunkHeight; y++) {
            for (int z = 0; z < chunksize; z++) {
                ...
                else       sideVisible[0] = !isOpaqueGlobal(x, y, chunksize - 1, front); 

              
                if (z < chunksize - 1) sideVisible[1] = (chunk.blocks[x][y][z + 1] == AIR);
                else                   sideVisible[1] = !isOpaqueGlobal(x, y, 0, back); 
                ...
                for (int s = 0; s < 6; s++) {
                    if (sideVisible[s]) { 
                        ...
                        for (int v = 0; v < 4; v++) {
                            ...
                            float OriginalU = unitCube[idx + 3]; 
                            float OriginalV = unitCube[idx + 4]; 
            ...
            for (int x = -half - 1; x <= half + 1; x++) {
                for (int z = -half - 1; z <= half + 1; z++) {
                    ...
                    if (chunks[ax][az].chunkPos != newPos) {
                        ...
                        chunks[ax][az].isGenerated = false; 
...
bool isSolid(float x, float y, float z) {
    ...
    return chunks[arrayX][arrayZ].blocks[lx][gy][lz] != AIR; 
}
